...
// Public, open-tracked Activations report. Serves the stored HTML and logs
// each open. (Reachable without a session — "/activation" is allowlisted in
// the auth proxy.) Mirrors /s/[token] exactly.
...
export async function GET(req: Request, ctx: { params: Promise<{ token: string }> }) {
  ...
  const res = await fetch(`${sbUrl}/rest/v1/activation_shares?token=eq.${encodeURIComponent(token)}&select=id,html,open_count,first_opened_at,expires_at`, { headers: hdr, cache: "no-store" });
  ...
  try {
    ...
    await fetch(`${sbUrl}/rest/v1/activation_shares?id=eq.${row.id}`, {
