...
// Influencer completion form: typed full name + drawn signature (plain
// canvas, pointer events, exported as PNG data URL) — same pattern as the
// media release SignForm.
...
export function AgreementSignForm({ token, influencerName, preview = false }: { token: string; influencerName: string; preview?: boolean }) {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const drawing = useRef(false);
  const [hasInk, setHasInk] = useState(false);
  const [f, setF] = useState({ agreed: false, signed_name: "" });
  ...
  async function submit() {
    ...
    setBusy(true);
    const d = await fetch("/api/influencer-agreements/sign", {
  ...
  if (done) {
    return (
      <div className="mt-6 rounded-xl bg-emerald-50 border border-emerald-200 p-6 text-center">
        ...
        <p className="text-[13px] text-emerald-700 mt-1.5 leading-relaxed">A copy of the signed agreement is on its way to your inbox.</p>
      </div>
    );
  }
  ...
  return (
    <div className="mt-6 border-t border-gray-100 pt-5 space-y-4">
      ...
      <label className="flex items-start gap-2.5 text-[13px] text-slate-700 font-medium cursor-pointer">
        ...
        <span>I have read, understood and agree to the terms of this Agreement *</span>
      </label>
      <div>
        <label className="text-[12px] font-semibold text-slate-600 block mb-1">Type your full name *</label>
        <input value={f.signed_name} onChange={e => setF(p => ({ ...p, signed_name: e.target.value }))} placeholder={influencerName} className={inp} />
      </div>
      <div>
      ...
      <button onClick={preview ? undefined : submit} disabled={busy || preview}
        ...
        {preview ? "Preview only — sending disabled" : busy ? "Saving…" : "Sign the agreement"}
      </button>
    </div>
  );
}
