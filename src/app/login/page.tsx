      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 w-full max-w-sm">
        ...
        {sent ? (
          <div className="text-center py-4">
            ...
            <button onClick={() => setSent(false)} className="text-[11px] text-indigo-600 hover:underline mt-4">Use a different email</button>
          </div>
        ) : (
          <>
            ...
            <input
              ...
              className="mt-1 w-full text-sm border border-gray-200 rounded-lg px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-indigo-300"
            />
            {err && <p className="text-[12px] text-rose-500 mt-2">{err}</p>}
            <button onClick={send} disabled={loading || !email.trim()}
              className="mt-4 w-full text-sm font-semibold text-white bg-indigo-500 hover:bg-indigo-600 disabled:opacity-40 rounded-lg py-2.5">
