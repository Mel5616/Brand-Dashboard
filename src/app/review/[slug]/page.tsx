...
// link (e.g. a packaging insert). Enter an email, get a real single-use
// discount code, then follow through to actually leave the review.
type Incentive = { brand: string; label: string; review_url: string; discount_type: string; discount_value: number; min_spend: number | null };
...
export default function ReviewIncentive() {
  ...
  const [result, setResult] = useState<{ code: string; review_url: string } | null>(null);
  ...
  async function submit() {
    ...
    setBusy(true); setErr("");
    const res = await fetch("/api/review-request", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ slug, email }) }).then(r => r.json()).catch(() => null);
    setBusy(false);
      ...
      <div className="w-full max-w-md bg-white rounded-2xl border border-gray-100 shadow-sm p-8 text-center">
        {loading ? (
          ...
          <>
            ...
            <p className="text-sm text-slate-500 mb-6">Leave us a review and get <strong>{amount} off</strong> your next order{incentive.min_spend ? ` (min. spend $${incentive.min_spend})` : ""}.</p>
            <input
