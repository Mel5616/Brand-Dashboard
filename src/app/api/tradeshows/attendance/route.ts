...
export async function GET() {
  ...
  const res = await fetch(`${sbUrl}/rest/v1/tradeshow_attendance?select=tradeshow_id,day,attendance&order=day`, { headers: hdr(), cache: "no-store" });
...
export async function POST(req: Request) {
  ...
  const row = { tradeshow_id: String(b.tradeshow_id), day: String(b.day), attendance: Math.max(0, Math.round(Number(b.attendance) || 0)) };
