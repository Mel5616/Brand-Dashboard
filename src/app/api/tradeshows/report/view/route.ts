...
// Serve a stored post-show report with the correct content-type so it RENDERS.
...
export async function GET(req: Request) {
  ...
  if (!id) return new Response("Missing tradeshow_id", { status: 400 });
  const sb = await createClient();
  const { data } = await sb.from("tradeshow_reports").select("html_url,file_name").eq("tradeshow_id", id).single();
  if (!data?.html_url) return new Response("Not found", { status: 404 });
  const res = await fetch(data.html_url, { cache: "no-store" });
  if (!res.ok) return new Response("Could not load the report", { status: 502 });

  const isPdf = /\.pdf($|\?)/i.test(data.html_url) || /\.pdf$/i.test(data.file_name || "");
  if (isPdf) {
