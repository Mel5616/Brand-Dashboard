...
import { candidates, createWinbackCode, sendWinbackEmail, sweepRecoveries, winbackCode, WINBACK, PRESETS, presetByKey, type SendRow, type Checkout } from "@/lib/winbackOffer";
...
export async function GET(req: Request) {
  ...
  const preset = presetByKey(u.searchParams.get("preset"));
  const [cands, hist] = await Promise.all([candidates(from, to, preset), rest("winback_offers?select=*&order=sent_at.desc&limit=500")]);
  ...
  const sent = new Map(sends.filter(s => s.campaign.endsWith(preset.key)).map(s => [s.checkout_id, s]));
  return NextResponse.json({ ok: true, from, to, needsSetup, preset: preset.key, presets: PRESETS.map(p => ({ key: p.key, label: p.label, gifts: p.gifts.map(g => ({ key: g.key, name: g.name, price: g.price })) })), config: { days: WINBACK.days },
...
export async function POST(req: Request) {
  ...
  const preset = presetByKey(b.preset);
  const campaign = `${from.slice(0, 7)}-${preset.key}`;
  ...
  const cands = await candidates(from, to, preset);
  ...
  for (const c of targets) {
    ...
    const made = await createWinbackCode(code, expiresAt, c.name || c.email, preset);
    if (!made.ok) { results.push({ email: c.email, ok: false, error: made.error }); continue; }
    const mail = await sendWinbackEmail(c, code, expiresAt, preset);
    if (!testTo) {
