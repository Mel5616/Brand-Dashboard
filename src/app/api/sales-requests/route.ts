...
export async function PATCH(req: Request) {
  ...
  if (b.status && b.status !== item.status) {
    ...
    if (item.requester_email) {
      ...
      // "delivered" reads as "completed" everywhere requester-facing — matches
      // the dashboard's renamed status button/label.
      const statusLabel = toStatus === "delivered" ? "completed" : toStatus.replace(/_/g, " ");
      ...
      await sendMail({
        ...
        subject: declined ? `Request declined: ${item.title}` : `Request update: ${item.title}, ${statusLabel}`,
        html: shell(`
          <p style="font-size:15px">Your ${item.request_type.replace(/_/g, " ")} request "<strong>${item.title}</strong>" is now <strong>${statusLabel}</strong>.</p>
