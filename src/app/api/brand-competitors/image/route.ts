...
// Upload a competitor product/brand photo to Supabase Storage and store it
// on the brand_competitors row. Admin only, same tier as editing the tracker.
export const revalidate = 0;
const BUCKET = "competitor-images";
...
export async function POST(req: Request) {
  ...
  const { error } = await sb.from("brand_competitors").update({ image_url: url }).eq("id", id);
