...
import { candidates, createWinbackCode, sendWinbackEmail, sweepRecoveries, winbackCode, WINBACK, GIFTS, type SendRow, type Checkout } from "@/lib/winback";
...
export async function GET(req: Request) {
  ...
  const [cands, hist] = await Promise.all([candidates(from, to), rest("winback_sends?select=*&order=sent_at.desc&limit=500")]);
...
export async function POST(req: Request) {
  ...
  const already = await rest(`winback_sends?campaign=eq.${encodeURIComponent(campaign)}&select=checkout_id`);
  if (!already.ok) return NextResponse.json({ ok: false, error: missing(await already.text()) ? "Run supabase/add_winback_sends.sql first" : "database error" }, { status: 500 });
  ...
  for (const c of targets) {
    ...
    if (!testTo) {
      await rest("winback_sends", { method: "POST", headers: { Prefer: "return=minimal" }, body: JSON.stringify({
