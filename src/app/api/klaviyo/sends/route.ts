...
import { createDraftCampaign, scheduleSend, cancelCampaign, campaignStats } from "@/lib/klaviyo";
...
export async function POST(req: Request) {
  ...
  if (b.action === "create") {
    const { subject, listId, listName, html } = b;
    if (!subject || !listId || !html) return NextResponse.json({ ok: false, error: "Missing subject, listId or html" }, { status: 400 });
    try {
      const { campaignId } = await createDraftCampaign({
        name: `${subject} — ${new Date().toLocaleDateString("en-AU")}`,
        listId, subject, fromEmail: "hello@coolkidz.com.au", fromLabel: "Coolkidz Australia", html,
      });
      const row = { campaign_id: campaignId, subject, list_id: listId, list_name: listName ?? null, html, status: "draft", created_by: access.user?.email ?? null };
