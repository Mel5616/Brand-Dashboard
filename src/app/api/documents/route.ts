...
// Documents admin API (Partnerships & Affiliates > Documents) — same pattern as
// Launch Decks. GET lists documents + share links + view stats. POST multipart
// creates a document from an uploaded .html file (or html field). PATCH:
// share.create / share.delete. DELETE removes a document.
...
export async function GET() {
  ...
  if (d.error) return NextResponse.json({ ok: true, needsSetup: missing(d.error.message), documents: [], shares: [], views: [] });
  return NextResponse.json({ ok: true, documents: d.data ?? [], shares: s.data ?? [], views: v.data ?? [] });
}
...
export async function POST(req: Request) {
  ...
  // Chunked upload for big documents (Vercel caps request bodies ~4.5MB):
  ...
  if (!title) return NextResponse.json({ ok: false, error: "Document title required" }, { status: 400 });
  ...
  if (!html.trim()) return NextResponse.json({ ok: false, error: "Attach the document's HTML file" }, { status: 400 });
  const sb = await createClient();
  const { data, error } = await sb.from("documents").insert({
  ...
  // Every document starts with a "Team" link for internal sharing
  await sb.from("document_shares").insert({ document_id: data.id, label: "Team" });
  return NextResponse.json({ ok: true, item: data });
}
...
export async function PATCH(req: Request) {
  ...
  if (b.action === "share.create") {
    ...
    if (!b.document_id || !label) return NextResponse.json({ ok: false, error: "Label required" }, { status: 400 });
    const { data, error } = await sb.from("document_shares").insert({ document_id: Number(b.document_id), label }).select().single();
    if (error) return NextResponse.json({ ok: false, error: error.message.slice(0, 150) }, { status: 500 });
    return NextResponse.json({ ok: true, share: data });
  }
  if (b.action === "share.delete") {
    const { error } = await sb.from("document_shares").delete().eq("id", Number(b.share_id));
