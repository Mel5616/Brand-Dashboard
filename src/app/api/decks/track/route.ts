...
// PUBLIC: view tracking for shared decks. "open" creates a view row for this
// browser session; "beat" accumulates viewing time (sent every 10s while the
// deck is actually visible). Logged-in dashboard users are identified by email;
// external viewers are anonymous per-link.
...
export async function POST(req: Request) {
  ...
  const sRes = await fetch(`${sbUrl}/rest/v1/deck_shares?token=eq.${token}&select=id&limit=1`, { headers: h(), cache: "no-store" });
  ...
  const cur = await fetch(`${sbUrl}/rest/v1/deck_views?share_id=eq.${share.id}&session_id=eq.${encodeURIComponent(session)}&select=id,seconds&limit=1`, { headers: h(), cache: "no-store" }).then(r => r.json()).catch(() => []);
  if (cur[0]) {
    await fetch(`${sbUrl}/rest/v1/deck_views?id=eq.${cur[0].id}`, {
