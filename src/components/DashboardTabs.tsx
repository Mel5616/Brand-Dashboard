...
import { OpeningOrders } from "./OpeningOrders";
...
const TABS: { id: TabId; label: string; icon: React.ReactNode }[] = [
  ...
  },
  {
    ...
    icon: <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v13m0-13a4 4 0 00-4-4 2 2 0 000 4h4zm0 0a4 4 0 014-4 2 2 0 010 4h-4zM4 12h16M5 21h14a1 1 0 001-1v-8H4v8a1 1 0 001 1z" /></svg>,
  },
  {
  ...
  },
  {
    ...
    icon: <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19V6l8 4v9M9 19H5a2 2 0 01-2-2v-6l6-3M9 19h8m0 0h2a2 2 0 002-2v-3" /></svg>,
  },
  {
  ...
  },
  {
    ...
    icon: <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 7l2-3h12l2 3M4 7h16M4 7v11a2 2 0 002 2h12a2 2 0 002-2V7M10 11h4" /></svg>,
  },
  {
  ...
  },
  {
    ...
    icon: <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a4 4 0 00-3-3.87M9 20H4v-2a4 4 0 013-3.87m6-1.13a4 4 0 100-8 4 4 0 000 8zm6 3c1.66 0 3 1.34 3 3v2h-6v-2c0-1.66 1.34-3 3-3zm-12 0c1.66 0 3 1.34 3 3v2H4v-2c0-1.66 1.34-3 3-3z" /></svg>,
  },
  {
  ...
  },
  {
    ...
    icon: <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h4m1-16H5a2 2 0 00-2 2v16a2 2 0 002 2h14a2 2 0 002-2V8z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 2v6h6" /></svg>,
  },
  {
  ...
  {
    id: "pa-revenue", label: "Revenue",
    icon: <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8V7m0 1v8m0 0v1M3 3v18h18" /></svg>,
  },
  {
...
const TAB_GROUPS: { label: string; ids: TabId[] }[] = [
  ...
  { label: "Operations", ids: ["budget", "expenses", "new-products", "product-info", "brand-assets", "stock-report", "cost-sheet", "credentials", "staff"] },
...
export function DashboardTabs({
  ...
  // Every tab is grantable to a member EXCEPT the ones in ADMIN_ONLY_TABS
  // (currently just Passwords) — those stay hidden from the nav here no
  // matter what's in allowedTabs, and the API route enforces the same
  // boundary again server-side as the real security check.
  const FINANCIAL: string[] = ADMIN_ONLY_TABS;
              ...
              <div key={g.label} ref={el => { sidebarGroupRefs.current[g.label] = el; }} className="mb-2">
                ...
                <div className="space-y-0.5">
                  ...
                  {flatTabs.map(tab => {
                    ...
                    const isOpen = !!nestedOpen[tab.id];
                    return (
                      <div key={tab.id}>
                        <div className="group/row relative flex items-center">
                          ...
                          <button onClick={(e) => { e.stopPropagation(); setNestedOpen(o => ({ ...o, [tab.id]: !o[tab.id] })); }} title={isOpen ? "Collapse" : "Expand"}
                        ...
                        {isOpen && <div className="ml-3 pl-1.5 border-l border-gray-200 space-y-0.5">{childTabs.map(Btn)}</div>}
                      </div>
                    );
                  })}
                </div>
    ...
    <>
      ...
      <div className="lg:ml-[288px]">
        <main className="max-w-screen-2xl mx-auto px-6 py-8 pb-24 lg:pb-8 space-y-8">
          ...
          {/* ── Create Website Codes: the one-code-on-every-site creator, split out from Cross Site Discounts ── */}
          {active === "create-codes" && (
            <>
              <SectionBar title="Create Website Codes" />
              <CrossCodeCard canCreate={role === "admin" || (allowedTabs ?? []).includes("create-codes")} />
            </>
          )}
          ...
          {/* ── Baby Registry: the registries parents create on uppababy.com.au ── */}
          {active === "registry" && (
            <>
              <SectionBar title="Baby Registry" />
              <RegistryPanel />
            </>
          )}
          ...
          {active === "reviews" && (
            <>
              <SectionBar title="Reviews" />
              <ReviewsPanel brands={brands.map((b: any) => ({ id: b.id, name: b.name }))} canEdit={role === "admin"} />
            </>
          )}
          ...
          {active === "utm-tracking" && (
            <>
              <SectionBar title="UTM Tracking" />
              <UtmTrackingPanel brands={brands.map((b: any) => ({ name: b.name }))} admin={role === "admin"} />
            </>
          )}
          ...
          {/* ── Amazon Ads (no live API — results uploaded from the Amazon Ads console) ── */}
          {active === "amazon-ads" && (
            <>
              <SectionBar title="Amazon Ads" />
              <AmazonAdsCard brands={brands} admin={role === "admin"} />
            </>
          )}
          ...
          {/* ── Show Insights ── */}
          {active === "show-insights" && <ShowInsights />}

          {/* ── Show Deals ── */}
          {active === "show-deals" && (
            <>
              <SectionBar title="Tradeshow Deals" />
          ...
          {/* ── Staff — who's off, synced from Connecteam ── */}
          {active === "staff" && (<>
            <SectionBar title="Staff" />
            <StaffCalendar />
          </>)}

          {active === "credentials" && role === "admin" && (<>
            <SectionBar title="Passwords" />
            <CredentialsPanel />
          </>)}
          ...
          {active === "stock-availability" && (<>
            <SectionBar title="Stock Availability" />
            <StockAvailability canEdit={role === "admin" || !!allowedTabs?.includes("stock-availability")} admin={role === "admin"} />
          </>)}

          {active === "order-forms" && (<>
            <SectionBar title="Order Forms" />
            <OpeningOrders canEdit={role === "admin" || !!allowedTabs?.includes("order-forms")} admin={role === "admin"} />
          </>)}
          ...
          {active === "decks" && (
            <>
              ...
              <LaunchDecks brands={brands.map((b: any) => ({ name: b.name }))} admin={role === "admin"} canManageLinks={role === "admin" || !!allowedTabs?.includes("decks")} />
            </>
          )}

          {active === "event-concepts" && (
            <>
              <SectionBar title="Event Concepts" />
              <EventConcepts brands={brands.map((b: any) => ({ name: b.name, color: b.color }))} admin={role === "admin"} />
            </>
          )}
          ...
          {active === "campaign-briefs" && (
            <>
              <SectionBar title="Campaign Briefs" />
              <CampaignBriefs brands={brands.map((b: any) => ({ id: b.id, name: b.name }))} admin={role === "admin"} />
            </>
          )}
