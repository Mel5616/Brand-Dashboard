  ...
  if (scope === "all") {
    ...
    return (
      <div className="space-y-5">
        {withPosts.map(b => {
          const posts = instagramMedia.filter(m => m.brand_id === b.id).slice(0, 6);
          return (
            <div key={b.id}>
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <Card label="Followers" value={num(latestFollowers(scope))} accent="#E1306C" />
        <Card label="Reach (latest mo.)" value={latest?.reach ? num(latest.reach) : "—"} accent="#3b82f6" />
        <Card label="Profile views" value={latest?.profile_views ? num(latest.profile_views) : "—"} accent="#8b5cf6" />
