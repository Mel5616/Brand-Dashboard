...
export function SocialPanel({
  ...
  const brandStats = (id: number) => {
    ...
    return { n, followers, growth, series: hist.slice(-6).map(d => d.followers), avgLikes: n ? likes / n : 0, avgComments: n ? comments / n : 0, rate };
  };

  // Photo / Reel / Carousel split with average engagement per type.
  const typeMix = (id: number) => {
    const groups: Record<string, number[]> = { Photos: [], Reels: [], Carousels: [] };
    for (const m of instagramMedia.filter(x => x.brand_id === id)) groups[postType(m)].push(eng(m));
  ...
  if (scope === "all") {
    ...
    // Leaderboard ranking
    const ranked = withPosts.map(b => ({ b, s: brandStats(b.id) }))
