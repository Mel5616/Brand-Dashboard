...
type PresetInfo = { key: string; label: string; gifts: Gift[] };
...
export function WinbackCard({ admin }: { admin: boolean }) {
  ...
  const [preset, setPreset] = React.useState("vista-cruz-four");
  const [data, setData] = React.useState<{ candidates: Cand[]; sends: Send[]; presets: PresetInfo[]; preset: string; needsSetup: boolean; live: boolean; config: { days: number } } | null>(null);
  ...
  const load = React.useCallback(() => {
    fetch(`/api/winback-offer?from=${from}&to=${to}&preset=${preset}`).then(r => r.json()).then(d => {
      if (!d.ok) return;
  ...
  async function send(test: boolean) {
    ...
    const body = test ? { from, to, preset, test_to: testTo, ids: [...sel].slice(0, 1) } : { from, to, preset, ids: [...sel] };
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 mt-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="text-[11px] font-bold uppercase tracking-[0.18em] text-slate-600">Win-back offers (UPPAbaby)</h2>
          <p className="text-xs text-gray-400 mt-0.5">People who left a pram at checkout and have not ordered since. Each send creates a one-use code on the store ({data?.config.days ?? 14} days) and emails a one-click link that rebuilds the bag with the gift added and the code applied.</p>
          <select value={preset} onChange={e => setPreset(e.target.value)} className={`${inp} mt-2`}>
            {(data?.presets ?? [{ key: "vista-cruz-four", label: "Vista / Cruz full price: four accessories free", gifts: [] }]).map(p => <option key={p.key} value={p.key}>{p.label}</option>)}
          </select>
        </div>
      ...
      {c.length === 0 ? <p className="text-[12.5px] text-gray-400">No open checkouts for this offer in the window.</p> : (
              ...
              <tbody>
                {sends.map(s => (
                  <tr key={s.id} className="border-b border-gray-50 align-top">
                    ...
                    <td className="py-2 pr-3 whitespace-nowrap text-gray-500">{s.campaign.replace(/^\d{4}-\d{2}-/, "")}</td>
